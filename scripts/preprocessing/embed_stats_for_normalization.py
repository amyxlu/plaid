...
def calc_stats(x, mask):
    ...
    channel_means, channel_stds = (
        channel_means.cpu().numpy(),
        channel_stds.cpu().numpy(),
    )
...
def main():
    ...
    def embed_batch_esmfold(sequences, embed_key_result):
        with torch.no_grad():
            embed_results = embedder.infer_embedding(
                sequences, return_intermediates=True
            )
    ...
    for batch in tqdm(dataloader):
        ...
        sequences = get_random_sequence_crop_batch(
            sequences, args.seq_len, args.min_len
        )
